...
- (id)init
{
	...
	if (self != nil)
	{
		g_stack = [[NSMutableArray alloc]init];
		clicked = [[NSMutableArray alloc]init];
		[self performSelector:@selector(clickButtons) withObject:nil afterDelay:1.0];
...
- (void)performTouchInView:(UIView *)view
{
	...
	[touch release];
	counter++;
	NSLog(@"Number of buttons pressed: %d",counter);
}
...
- (NSArray *)findButtons:(NSString *)xpath
{
...
// snapshot
//
// Finds all buttons in current view not previously clicked on and pushes 
// them onto stack for clicking.
//
- (void)snapshot:(NSMutableArray *)stack
{
	...
	NSArray *rectButtons = [self viewsForXPath:@"//dict[key[.='className' and following-sibling::string[1]/.='UIRoundedRectButton']]"];
	for (UIView *e in rectButtons) {
		if (![stack containsObject:e] && ![clicked containsObject:e]) {
			[stack addObject:e];
		}
	}
...
// clickButtons
//
// Finds all buttons and clicks them.
//
- (void)clickButtons
{
	static int counter = 0;
	...
	[self performTouchInView:[g_stack objectAtIndex:[g_stack count]-1]];
	[clicked addObject:[g_stack objectAtIndex:[g_stack count]-1]];
	[g_stack removeLastObject];
	if (![g_stack count] == 0) {
